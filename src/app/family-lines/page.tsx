...
const buildFamilyForest = () => {
  const bigToLittlesMap = new Map<string, string[]>();
  ...
  const buildTree = (name: string): Member => {
  ...
  const forest: FamilyTree[] = roots.map(root => ({
    root: buildTree(root)
  }));
...
const FamilyLinesPage = () => {
  ...
  useEffect(() => {
    // Build the forest
    const { forest, roots } = buildFamilyForest();
    setForest(forest);
    setRoots(roots);
